...
const validateForm = () => {
    ...
    let artesaniaSelect = document.getElementById("artesanias"); 
    let selectedOptions = Array.from(artesaniaSelect.selectedOptions);
    ...
    if (!validateOption(option)) {
        setInvalidInput(`Debes escoger de 1 a 3 tipos de artesanias`);
    }
    if (!validateFiles(files)) {
        setInvalidInput('Fotos de artesanias')
    }
    if (!validateComent(descripcion)) {
    ...
    if (!isValid) {
        ...
        const registrarArtesanoBtn = document.getElementById("registrar-artesano");
        registrarArtesanoBtn.style.display = "none";
...
let RegistrarBtn = document.getElementById("registrar-artesano");
...
enviarFormularioBtn.addEventListener("click", () => {
    ...
    const myForm = document.forms["artesano-form"];
    myForm.submit();
});
